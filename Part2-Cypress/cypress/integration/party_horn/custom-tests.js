describe('Party Horn Tests', () => {
  ...
  it('Honk button is disabled when volume text box is set to text', () => {
  ...
  it('Error is thrown when number >100 is typed into input field', () => {
    cy.get('#volume-number')
      .invoke('val', 300)
      .trigger('input');
    cy.get('#volume-number').then($el => {
      expect($el[0].validationMessage).to.contains("Value must be");
    });
  });
});